...
def test_update_order_add_item(
    api_request_context: APIRequestContext, sample_order, another_product
):
    payload = {
        "items": [
    ...
    response = api_request_context.put(
        f"/api/orders/{sample_order['id']}/", data=payload
    )
...
def test_update_order_restore_stock(
    api_request_context: APIRequestContext, sample_order, sample_product
):
    ...
    refreshed_product = api_request_context.get(
